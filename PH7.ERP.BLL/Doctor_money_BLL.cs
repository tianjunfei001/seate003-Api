...
namespace PH7.ERP.BLL
{
    public class Doctor_money_BLL:BaseBLL
    {
        ...
        /// <param name="Doctor_ID"></param>
        /// <returns></returns>
        public List<Doctor_bankcard_Model> GetBindGetBank()
        {
            //int id, int Doctor_ID
            string sql = $"select Doctor_bankcard.* from Doctor_money  join Doctor_bankcard on Doctor_money.id = Doctor_bankcard.Doctor_moneyId";
            DataSet ds = helper.GetDataSet(sql);
            List<Doctor_bankcard_Model> list = GetHelper.DatatableTolist<Doctor_bankcard_Model>(ds.Tables[0]);
            return list;
        }
